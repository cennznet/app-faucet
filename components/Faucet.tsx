...
const Faucet: FC = () => {
  ...
  const onTokenChange =
    (event: SelectChangeEvent) => {
      const value = event.target.value;
      setToken(
        supportedTokens.find(token => token.symbol === value)
      );
    };
    ...
    <div css={styles.faucetWrapper}>
      <div css={styles.faucetContainer}>
        <div css={styles.headingContainer}>
          ...
          <TokenSelect selectedToken={token.symbol} onTokenChange={onTokenChange} />
          <NetworkSelect selectedNetwork={network} onNetworkChange={onNetworkChange} />
        </div>
        <Divider />
